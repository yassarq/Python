...
class QuoteManager(models.Manager):
    def quoteValidator(self, form, id):
        ...
        if not errors:
            # save user in database
            errors.append("Successfully Added a Quote")
            quote = Quote.objects.create(quotename = quotename, uploaded_by = User.objects.get(id=id))
            return (True, quote, errors)
        else:
            return (False, errors)
...
class User(models.Model):
    ...
    # uploadedquote -> joins to Quote table (uploaded_by)
    # likedquote -> joins to Quote table (likers)
...
class Quote(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    uploaded_by = models.ForeignKey(User, related_name="uploadedquote")
    likers = models.ManyToManyField(User, related_name="likedquote")

    objects = QuoteManager()

    def __repr__(self):
        return "<Quote: {}>".format(self.quotename)